...
class DetailView extends ListView {
	...
	function processListNavigation( &$xtpl, $html_varName, $current_offset, $display_audit_link = false){
		...
		$html_text .= "<td COLSPAN=\"20\">\n";
        $html_text .= "<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\"><tr><td style=\"text-align: left\" >&nbsp;".$audit_link."</td>\n";
        ...
        if ($this->return_to_list_only == true) {
	        ...
	        if($current_offset != 0 && $this->isRequestFromListView($html_varName))
	        {
				...
				///////////////////////////////////////////////////////////////
				////	RETURN TO LIST URL
	    		// cn: bug 9329 - detail nav loses type var (group, inbound, archive, etc)
	    		$list_URL = $this->base_URL.'&action=index&module='.$_REQUEST['module'];
	    		if($this->local_current_module == 'Emails') {
	    			if(!isset($_SESSION['emailStartAction']) || empty($_SESSION['emailStartAction'])) {
		    			// require
		    			
		    			$uri = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'index.php';
						$start = Email::getStartPage($uri);
						if (!isset($start['action'])) {
							$start['action'] = $uri;
						} // if
						$_SESSION['emailStartAction'] = (isset($start['current_view'])) ? $start['current_view'] : $start['action'];
	    			}
	    			//$list_URL = $this->base_URL."&action={$_SESSION['emailStartAction']}&module={$_REQUEST['module']}";
	    			$extraListURLParam = "";
	    			if (strstr($_SESSION['emailStartAction'], "type=archived") !== FALSE) {
	    				$extraListURLParam = "&action=ListView&assigned_user_id={$current_user->id}&type=archived";
	    			} else {
	    				$extraListURLParam = "&action=ListViewAll&all=true";
	    			} // else
	    			$list_URL = "index.php?module=Emails".$extraListURLParam;
	    		}
				////	RETURN TO LIST URL
				///////////////////////////////////////////////////////////////
				...
				$list_link = "<button type='button' class='button' title='{$GLOBALS['app_strings']['LNK_LIST_RETURN']}' onClick='location.href=\"$list_URL\";'>".$this->local_app_strings['LNK_LIST_RETURN']."</button>";
	   			...
	   			$html_text .= "<td nowrap align='right' >".$list_link."&nbsp;&nbsp;&nbsp;&nbsp;".$start_link."&nbsp;&nbsp;".$previous_link."&nbsp;&nbsp;(".$current_offset." ".$this->local_app_strings['LBL_LIST_OF']." ".$row_count.")&nbsp;&nbsp;".$next_link."&nbsp;&nbsp;".$end_link."</td>";
