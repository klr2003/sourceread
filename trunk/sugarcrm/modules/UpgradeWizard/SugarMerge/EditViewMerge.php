	...
	protected function mergeFields(){
		foreach($this->customFields as $field=>$data){
			...
			if(isset($this->newFields[$field]) && isset($this->originalFields[$field])){
				//if both the custom field and the original match then we take the location of the new field since it hasn't moved
				if($this->areMatchingValues($this->customFields[$field]['loc'], $this->originalFields[$field]['loc'])
				//AND the custom field and new field location don't match 
				&& !$this->areMatchingValues($this->customFields[$field]['loc'], $this->newFields[$field]['loc'])){
						//we take the location of the new field
						$loc = $this->newFields[$field]['loc'];
						$loc['source'] = 'new';
				}else{
						//othewise we take the location of the custom field
