...
function changeQS() {
    ...
    if(typeof(disabledModules[new_module]) != 'undefined') {
        sqs_objects['ImportEditView_parent_name']['disable'] = true;
        document.getElementById('parent_name').readOnly = true;
        document.getElementById('parent_name').value = mod_strings['LBL_QS_DISABLED'];
    }
    else {
        sqs_objects['ImportEditView_parent_name']['disable'] = false;
        document.getElementById('parent_name').readOnly = false;
    }   
    sqs_objects['ImportEditView_parent_name']['modules'] = new Array(new_module);
    if (document.getElementById('smartInputFloater')) document.getElementById('smartInputFloater').style.display = 'none';
    //var newArray = array();
    QSFieldsArray['ImportEditView_parent_name'].sqs.modules = new Array(new_module);
    /*
	var qsFields = YAHOO.util.Dom.getElementsByClassName('sqsEnabled');
	for(i = 0 ; i < qsFields.length ; i++){
		var qsField = qsFields[i];
        if(qsField != null && (typeof Ext.getCmp('combobox_'+qsField.id) != 'undefined')) {
        	var comboBoxObject = Ext.getCmp('combobox_'+qsFields[i].id);
        	var baseParamData = Ext.util.JSON.decode(comboBoxObject.store.baseParams.data);
        	baseParamData.modules = new Array(new_module);
        	comboBoxObject.store.baseParams.data = Ext.util.JSON.encode(baseParamData);
        } // if
	} // for
	*/
    enableQS(true);
...
<select onchange=" document.ImportEditView.parent_name.value=''; checkParentType(document.ImportEditView.parent_type.value, document.ImportEditView.change_parent); changeQS();" name="parent_type" id="parent_type" tabindex="2">
