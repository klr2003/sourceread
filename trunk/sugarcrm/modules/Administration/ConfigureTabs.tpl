...
<form name="ConfigureTabs" method="POST"  method="POST" action="index.php">
	...
	<table border="0" cellspacing="1" cellpadding="1">
		<tr>
			<td>
				<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="button" onclick="save();this.form.action.value='SaveTabs'; " type="submit" name="button" value="  {$APP.LBL_SAVE_BUTTON_LABEL}  " > 
	...
	<div class='add_table' style='margin-bottom:5px'>
		<table id="ConfigureTabs" class="themeSettings edit view" style='margin-bottom:0px;' border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td><span><b>{$MOD.LBL_VISIBLE_TABS}</b></span></td>
				<td><span><b>{$MOD.LBL_HIDDEN_TABS}</b></span></td>
			</tr>
			<tr>
				<td width='1%'>
					<div id="enabled_div" class="enabled_tab_workarea">
						<ul id="enabled_ul" class="module_draglist">
						{foreach from=$enabled_tabs key=dirname item=name}
				...
				<td>
					<div id="disabled_div" class="disabled_tab_workarea">
						...
						{foreach from=$disabled_tabs key=dirname item=name}
							<li id="{$dirname}" class="noBullet2">{$name}</li>
						{/foreach}
						</ul>
					</div>
...
{/literal}	    
	...
	{foreach from=$enabled_tabs key=module item=moduleDisplay}
    {if $module != $currentTheme}new YAHOO.example.DDList("{$module}");{/if}
	{/foreach}	
	 
	{foreach from=$disabled_tabs key=module item=moduleDisplay}
