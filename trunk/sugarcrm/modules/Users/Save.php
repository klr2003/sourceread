...
if(!$current_user->is_admin  && !is_admin_for_module($GLOBALS['current_user'],'Users')&& !empty($_POST['is_admin'])) {
	...
	if(!$focus->is_group && !$focus->portal_only){	
	    ...
	    {
	        $focus->setPreference('module_favicon', $_POST['user_module_favicon'], 0, 'global', $focus);
	    }
	    else
	    {
	        $focus->setPreference('module_favicon', '', 0, 'global', $focus);
	    }
	    ...
	    foreach($_POST as $k=>$v){
	        if(strpos($k,"sugarpdf_pdf") !== false){
	            $focus->setPreference($k, $v, 0, 'global', $focus);
	        }
	    }
	    ////    PDF SETTINGS
		///////////////////////////////////////////////////////////////////////////
