...
function send_back(module, id)
{
	...
	for(var the_key in field_to_name_array)
	{
		if(the_key != 'toJSON')
		{
			...
			if(module != '' && id != '')
			{
                if(the_key.toUpperCase() == 'USER_NAME' && typeof(is_show_fullname) != 'undefined' && is_show_fullname && form_name != 'search_form') {//if it is from searchform, it will search by assigned_user_name like 'ABC%', then it will return nothing
