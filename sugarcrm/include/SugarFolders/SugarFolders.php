...
class SugarFolder {
	...
	// core queries
	var $core = "SELECT f.id, f.name, f.has_child, f.is_group, f.is_dynamic, f.dynamic_query, f.folder_type, f.created_by FROM folders f ";
	var $coreSubscribed = "SELECT f.id, f.name, f.has_child, f.is_group, f.is_dynamic, f.dynamic_query, f.folder_type, f.created_by FROM folders f LEFT JOIN folders_subscriptions fs ON f.id = fs.folder_id ";
	...
	function setFolder($fields) {
		...
		if(empty($fields['groupFoldersUser'])) {
			$fields['groupFoldersUser'] = $current_user->id;//createGroupUser($fields['name']);
		}
	...
	function setSubscriptions($subs) {
		...
		foreach($cleanSubscriptions as $id) {
			$guid = create_guid();
			$q = "INSERT INTO folders_subscriptions (id, folder_id, assigned_user_id) VALUES ('{$guid}', '{$id}', '{$current_user->id}')";
			$r = $this->db->query($q);
		}
	}
	...
	function getListItemsForEmailXML($folderId, $page = 1, $pageSize = 10, $sort = '', $direction='') {
		...
		if($this->is_dynamic) {
			...
			//$q = "SELECT * FROM folders_rel WHERE folder_id = '{$folderId}' AND deleted = 0";
		...
		while($a = $this->db->fetchByAssoc($r)) {
			//if(empty($a['polymorphic_module']))
			//	continue;

			//$class = $beanList[$a['polymorphic_module']];
			//require_once("modules/{$a['polymorphic_module']}/{$class}.php");

			//$beanTemp = new $class();
			//$beanTemp->retrieve($a['polymorphic_id']);

			//if(empty($beanTemp->id))
			//	continue; // prevent bad rows from showing up in the grid

			$temp = array();
			$temp['flagged'] = ($a['flagged'] == '0') ? '' : 1;
			$temp['status'] = ($a['reply_to_status'] == '0') ? '' : 1;
	...
	function addBean($bean, $user=null) {
		...
		// Samir Gandhi : This code is not needed anymore
		/*
		if($this->assign_to_id != "") {
			$bean->assigned_user_id = $this->assign_to_id;
			$bean->save(false);
		}*/
	}
	...
	function retrieveFoldersForProcessing($user, $subscribed=true) {
		...
		if($subscribed) {
			$q = $this->coreSubscribed.$this->coreWhereSubscribed."'{$user->id}' ".$rootWhere.$this->coreOrderBy;
		} else {
			$q = $this->core.$this->coreWhere.$rootWhere.$this->coreOrderBy;
		}
		$r = $this->db->query($q);
		$return = array();
	...
	function deleteChildrenCascade($id) {
		...
		$canContinue = true;
		$checkInboundQuery = "SELECT count(*) c FROM inbound_email WHERE groupfolder_id = '{$id}'";
	...
	function save() {
		...
		if((empty($this->id) && $this->new_with_id == false) || (!empty($this->id) && $this->new_with_id == true)) {
			...
			$q = "INSERT INTO folders(id, name, folder_type, parent_folder, has_child, is_group, is_dynamic, dynamic_query, assign_to_id, ".
				...
				" VALUES('{$guid}', '{$this->name}', '{$this->folder_type}', '{$this->parent_folder}', {$this->has_child}, {$this->is_group}, {$this->is_dynamic}, '{$this->dynamic_query}', '{$this->assign_to_id}', " .
			...
			$guid2 = create_guid();
			$q2 = "INSERT INTO folders_subscriptions VALUES('{$guid2}', '{$guid}', '{$current_user->id}')";
			$r2 = $this->db->query($q2);
