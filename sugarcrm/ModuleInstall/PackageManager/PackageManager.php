    ...
    function getPackagesInStaging($view = 'module'){
        ...
        foreach($upgrade_contents as $upgrade_content)
        {
            ...
            if(0 == sizeof($md5_matches))
            {
        		...
        		$packages[] = array('name' => $name, 'version' => $version, 'published_date' => $published_date, 'description' => $description, 'uninstallable' =>$uninstallable, 'type' => $type, 'file_install' => fileToHash($file_install), 'file' => fileToHash($upgrade_content));       
            }//fi
        }//rof
        return $packages;
    }
