...
*/
	...
	*/
	...
	function preprocess_fields_on_save()
	{
		...
		foreach($this->field_defs as $fieldDef)
		{
            if ( !isset($this->$fieldDef['name']) )
			{
				continue;
			}

            if ( is_string($fieldDef) && ( $fieldDef == 'date_modified' || $fieldDef == 'date_entered') )
                continue;
			...
			{
				case 'date':
					if($this->process_save_dates && empty($disable_date_format) && !empty($this->$field))
					{
						if(!empty($this->field_name_map[$field]['rel_field']))
						{
							$rel_field = $this->field_name_map[$field]['rel_field'];
	...
	function unformat_all_fields()
	{
		...
		if(!empty($this->unformated_numbers) || !empty($disable_num_format)  || empty($current_user)) return;
		...
		foreach($this->field_defs as $fieldDef)
		{
			...
			} else if($type=='encrypt' && !empty($this->$field)) {
			...
			if(empty($fieldDef['disable_num_format']) && !empty($this->$field))
			{
				if(in_array($type, array('int', 'uint', 'ulong', 'long', 'short', 'tinyint'))) {
                    ...
                    if ( isset($fieldDef['precision']) )
                        $this->$field = format_number($this->$field, $fieldDef['precision'], $fieldDef['precision']);
                    else
