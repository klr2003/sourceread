...
class User extends Person {
    ...
    function getSystemUser() {
        if(null !== $this->retrieve('1')) {
            ...
            $q = "SELECT users.id FROM users WHERE users.status = 'Active' AND users.deleted = 0 AND users.is_admin = 1";
            $r = $this->db->query($q);
	...
	function getSignature($id) {
		...
		$q = "SELECT id, signature, signature_html FROM users_signatures WHERE user_id = '{$this->id}' AND id='{$id}' AND deleted=0";
		$r = $this->db->query($q);
	 ...
	 */
	function getSignatures($live=false, $defaultSig='', $forSettings=false) {
		...
		$signs  = "<select {$change} id='{$id}' name='{$id}'>";
	...
	/**
	 * Alias {@link UserPreferences::setPreference()}
	 *
	 * @param string $name Name of the preference to set
	 * @param string $value Value to set preference to
	 * @param null $nosession For BC
	 * @param string $category Name of the category to retrieve
	 * @param User|null $user {@link User} to change the preference on, defaults to
	 *		                $this if left NULL
	 *
	 * @see UserPreference::setPerference()
	 */
	...
	/**
	 * Aliases {@link UserPreferences::resetPreferences()}
	 *
	 * NOTE: The parameter order is switched from that in
	 * {@link UserPreferences::resetPreferences()}
	 *
	 * @param User|null $user A {@link User} object, or NULL is using $this
	 * @param string|null $category Category to reset, or NULL if resetting all prefs
	 *
	...
	*/
	...
	function getEmailLink2($emailAddress, &$focus, $contact_id='', $ret_module='', $ret_action='DetailView', $ret_id='', $class='') {
		...
		if($client == 'sugar') {
			...
			$emailLink = '<a href="index.php?module=Emails&action=Compose'.
	...
	function getPrivateTeamID() {
		$q = "SELECT id FROM teams WHERE associated_user_id = '{$this->id}'";
		$r = $this->db->query($q);
		while($a = $this->db->fetchByAssoc($r)) {
			if(!empty($a['id'])) {
				return $a['id'];
