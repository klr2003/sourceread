...
class ImportFieldSanitize
{
        ...
        )
    {
        ...
        if ( $vardef['name'] != 'date_entered' && $vardef['name'] != 'date_modified')
            // convert back to user's timezone
            $value = $timedate->handle_offset(
                $value, $timedate->get_date_time_format(), true);
        else
            // in gmt, just change datetime format
            $value = $timedate->swap_formats(
        ...
        )
    {
        ...
        ereg($reg['format'], $value, $dateparts);
