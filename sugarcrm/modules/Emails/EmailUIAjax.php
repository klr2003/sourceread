  ...
  *********************************************************************************/
  ...
  switch($_REQUEST['emailUIAction']) {
    ...
    case "fillComposeCache": // fills client-side compose email cache with signatures and email templates
        ...
        $out = array();
        $out['emailTemplates'] = $email->et->getEmailTemplatesArray();
    ...
    case "getRelateForm":
    	if (isset($_REQUEST['uid']) && !empty($_REQUEST['uid'])) {
            ...
            $ret = $email->et->getImportForm(array('showTeam' => false, 'showAssignTo' => false, 'showDelete' => false), $email);
            $out = trim($json->encode($ret, false));
        echo $out;
    	}
    break;
    ...
    case "getMessageListXML":
        $GLOBALS['log']->debug("********** EMAIL 2.0 - Asynchronous - at: getMessageListXML");
        if(isset($_REQUEST['ieId']) && !empty($_REQUEST['ieId'])) {
            ...
            $out = $email->et->xmlOutput($list, 'Email', $count, true, $unread);
    ...
    case "getMessageListSugarFoldersXML":
        $GLOBALS['log']->debug("********** EMAIL 2.0 - Asynchronous - at: getMessageListSugarFoldersXML");
        if(isset($_REQUEST['ieId']) && !empty($_REQUEST['ieId'])) {
            // user view preferences
            ...
            $out = $email->et->xmlOutput($metalist, 'Email', $count, false, $unread);
    ...
    case "addGroupFolder":
        $GLOBALS['log']->debug("********** EMAIL 2.0 - Asynchronous - at: addGroupFolder");
        ...
        $email->et->folder->setFolder($_REQUEST);
        break;
        ...
        if(isset($_REQUEST['folderType']) && !empty($_REQUEST['folderType'])) {
            switch($_REQUEST['folderType']) {
                ...
                case "imap":
                    $ie->retrieve($_REQUEST['ieId']);
                    $ie->connectMailserver();
                    $status = $ie->deleteFolder($_REQUEST['mbox']);
                    if ($status == true) {
                    	$return['status'] = true;
    ...
    case "getGroupFolder":
        ...
        case "saveGroupFolder":
            $email->et->folder->retrieve($_REQUEST['record']);
            ...
            $ret = $email->et->folder->updateFolder($_REQUEST);
    ...
    case "saveOutbound":
        ...
        $ret['newId'] = $oe->id;
        $ret['mailers'] = $oe->getUserMailers($current_user);
        $out = $json->encode($ret);
    ...
    case "deleteIeAccount":
        ...
        if(isset($_REQUEST['group_id']) && $_REQUEST['group_id'] == $current_user->id) {
            $ret = array();
        	$ret['fromAccounts'] = $email->et->getFromAccountsArray($ie);
    ...
    case "searchAdvanced":
        ...
        $_REQUEST['dateTo'] != ""
            ...
            $metalist = $ie->search($_REQUEST['ieId'], $_REQUEST['subject'], $_REQUEST['from'], $_REQUEST['searchTo'], $_REQUEST['body'], $_REQUEST['dateFrom'], $_REQUEST['dateTo']);
    ...
    case "saveSettingsGeneral":
        ...
        $emailSettings['layoutStyle'] = $_REQUEST['layoutStyle'];
        //$emailSettings['autoImport'] = isset($_REQUEST['autoImport']) ? '1' : '0';
        $emailSettings['alwaysSaveOutbound'] = isset($_REQUEST['alwaysSaveOutbound']) ? '1' : '0';
        ...
        $emailSettings['tabPosition'] = isset($_REQUEST['tabPosition']) ? 'bottom' : 'top';
        $emailSettings['showNumInList'] = $_REQUEST['showNumInList'];
        $emailSettings['defaultOutboundCharset'] = $_REQUEST['default_charset'];
        $emailSettings['fullScreen'] = isset($_REQUEST['fullScreen']) ? '1' : '0';
   ...
   break;
    ...
    case "getAddressSearchResults":
        ...
        $qArray = $email->et->_getPeopleUnionQuery($wheres,$person, $start, 25, true);
        ...
        $time = microtime(true);
        $r = $ie->db->limitQuery($qArray['query'] , $start, 25, true);
        ...
        $ret = $email->et->xmlOutput(array('out' => $out), 'Person', $count);
    ...
    case "getReportUsers":
    	...
    	$out = $email->et->getReportUsers($reportId);
        $ret = $email->et->xmlOutput(array('out' => $out['data']), 'Person', $out['count']);
