	...
	function email2Send($request) {
        ...
        if ($this->type != 'draft') {
            ...
            if (@!$mail->Send()) {
                ...
                if((isset($mail->smtp->error) && !empty($mail->smtp->error)) || $mail->isHostEmpty) {
                	echo $mod_strings['LBL_SEND_EMAIL_FAIL'];
            	}  else if (!$mail->opensslOpened) {
            	    echo $mod_strings['LBL_OPENSSL_EXTENSION_NOT_OPEN'];
            	} else {
