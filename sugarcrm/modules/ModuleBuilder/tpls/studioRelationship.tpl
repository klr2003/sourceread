	...
	<tr>
		<td colspan='2' style="padding:5px 5px 15px 5px">
			...
			{if !$rel.readonly}
				...
				{/if}
				ModuleBuilder.submitForm("relform");ModuleBuilder.tabPanel.removeTab(ModuleBuilder.findTabById("relEditor"));' class='button'>
				{if ! empty($view_package)}
				&nbsp;
				{/if}
			{/if}
			...
			onclick='if(confirm("{$mod_strings.LBL_CONFIRM_RELATIONSHIP_DELETE}")){ldelim} 
				this.form.action.value="DeleteRelationship";ModuleBuilder.submitForm("relform");ModuleBuilder.tabPanel.removeTab(ModuleBuilder.findTabById("relEditor"));{rdelim}' class='button'>
            {/if}
	...
	<tr><td colspan=2>
		<table class="edit view">
			...
			<tr>
                ...
                <td><input name="lhs_label" id="lhs_label" value="{$rel.lhs_label}"  {if $rel.readonly}disabled{/if}></td>
                {else}<td></td><td></td>{/if}
                <td></td>
                {if $rel.relationship_type != 'many-to-one'} 
                <td align="right" scope="row">{sugar_translate label="LBL_REL_LABEL"}:</td>
                <td><input name="rhs_label" id="rhs_label" value="{$rel.rhs_label}"  {if $rel.readonly}disabled{/if}></td>
                {/if}
            </tr>
            <tr>
                {if $rel.relationship_type == 'many-to-many' || $rel.relationship_type == 'many-to-one'}
