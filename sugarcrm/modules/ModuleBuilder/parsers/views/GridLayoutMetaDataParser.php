...
class GridLayoutMetaDataParser extends AbstractMetaDataParser implements MetaDataParserInterface
{
    ...
    function getAvailableFields ()
    {
        ...
        foreach ( $this->_fielddefs as $key => $def )
        {
            ...
            {
                $availableFields [ $key ] = array ( 'name' => $key , 'label' => $def [ 'label' ] ) ; // layouts use 'label' not 'vname' for the label entry
            }
        ...
        for ( $pass=1 ; $pass<=2 ; $pass++ )
        {
        	foreach ( $_REQUEST as $slot => $value )
        	{
            	...
            	if ($slotComponents [ 0 ] == 'slot')
            	{
                	...
                	//If this field has a custom definition, copy that over, ignoring any (filler) values that might be in there
                	if ( $pass == 1 )
                	{
                		if ( $property == 'name' && $value != $this->FILLER['name'] )
	...
	{
		return array_intersect_key ( $def , array ( 'studio' => true , 'name' => true , 'label' => true , 'displayParams' => true , 'comment' => true , 'customCode' => true , 'customLabel' => true , 'tabIndex' => true , 'hideLabel' => true) ) ;
	}
}
