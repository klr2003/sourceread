...
<form name="ConfigureSubPanels" method="POST">
	...
	<div class='add_table' style='margin-bottom:5px'>
		<table id="ConfigureSubPanels" class="themeSettings edit view" style='margin-bottom:0px;' border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td><span><b>{$MOD.LBL_VISIBLE_PANELS}</b></span></td>
				<td><span><b>{$MOD.LBL_HIDDEN_PANELS}</b></span></td>
			</tr>
			<tr>
				<td width='1%'>
					<div id="enabled_div" class="enabled_panel_workarea">
						<ul id="enabled_ul" class="module_draglist">
						{foreach from=$enabled_panels key=dirname item=name}
				...
				<td>
					<div id="disabled_div" class="disabled_panel_workarea">
						<ul id="disabled_ul" class="module_draglist">
						{foreach from=$disabled_panels key=dirname item=name}
							<li id="{$dirname}" class="noBullet2">{$name}</li>
						{/foreach}
						</ul>
					</div>
...
function save() {
    ...
    document.ConfigureSubPanels.enabled_panels.value = enabled_display_vals != '' ? enabled_display_vals.substr(1,enabled_display_vals.length) : '';
    ...
    document.ConfigureSubPanels.disabled_panels.value = disabled_display_vals != '' ? disabled_display_vals.substr(1,disabled_display_vals.length) : '';
}
...
{/literal}	    
	...
	{foreach from=$enabled_panels key=module item=moduleDisplay}
    {if $module != $currentTheme}new YAHOO.example.DDList("{$module}");{/if}
	{/foreach}	
	 
	{foreach from=$disabled_panels key=module item=moduleDisplay}
