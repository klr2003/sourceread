...
//			_pp($UIDLs);_pp($cacheUIDLs);
	...
	function findOptimumSettings($useSsl=false, $user='', $pass='', $server='', $port='', $prot='', $mailbox='') {
		...
		if(!empty($retArray['good'])) {
			...
			foreach($raw as $mbox) {
				...
				//$raw[$i] = str_replace($testConnectString, "", $mbox);
				$i++;
			} // foreach
			sort($raw);
	...
	function getMessageText($msgNo, $type, $structure, $fullHeader,$clean_email=true) {
					...
					foreach($bcArr as $type => $bcTrail) {
						if($type == 'html') {
							...
							if(count($this->inlineImages > 0)) {
								...
								$msgPartRaw .= "<img class='image' src='{$sugar_config['site_url']}/{$sugar_config['cache_dir']}/images/{$imageName}'>";
							}
						}
					}
				}
		...
		// remove image tag which contains cid tag as cleanContent just removes cid tag in img tag and leaves
		// img tag as it is
		$msgPart = preg_replace("/<\s*img[^>]+src\s*=[^>]*?cid:[^>]*?>/sim", '', $msgPart);
	...
	function importRaw($msgNo) {
		...
		if(isset($sugar_config['email_inbound_save_raw']) && $sugar_config['email_inbound_save_raw'] == true) {
			...
			if($strlenAll <= 65535) {
				...
				$orig_func_overload=ini_get('mbstring.func_overload');
				...
				ini_set($orig_func_overload);
			}

			// safe the content
			if(isset($sugar_config['email_preserve_raw']) && $sugar_config['email_preserve_raw'] == 1) {
				$GLOBALS['log']->debug("*** INBOUNDEMAIL not cleaning RAW due to config settings.");
			} else {
				$raw = $this->cleanContent($raw);
			}
		} else {
			$GLOBALS['log']->debug("*** INBOUNDEMAIL not saving raw due to system settings");
			$raw = '';
		}
...
//
	...
	function getAutoreplyStatus($from) {
		...
		$maxReplies = (isset($sugar_config['email_num_autoreplies_24_hours']) && !empty($sugar_config['email_num_autoreplies_24_hours'])) ? $sugar_config['email_num_autoreplies_24_hours'] : 10;
	...
	function saveInboundEmailSystemSettings($type, $macro) {
		...
		// raw source setting
		$sugar_config['email_inbound_save_raw'] = (isset($_REQUEST['email_inbound_save_raw']) && $_REQUEST['email_inbound_save_raw'] == 1) ? true : false;

		// email_num_autoreplies_24_hours
		$sugar_config['email_num_autoreplies_24_hours'] = (isset($_REQUEST['email_num_autoreplies_24_hours']) && !empty($_REQUEST['email_num_autoreplies_24_hours'])) ? $_REQUEST['email_num_autoreplies_24_hours'] : 10;
...
eoq;
	...
	function retrieveAllByGroupId($id, $includePersonal=true) {
		...
		$q = "SELECT id FROM inbound_email WHERE is_personal = 0 AND groupfolder_id is null AND mailbox_type not like 'bounce' AND deleted = 0 AND status = 'Active' {$extraWhere}";
		$r = $this->db->query($q, true);
	...
	function retrieveAllByGroupIdWithGroupAccounts($id, $includePersonal=true) {
		...
		$q = "SELECT id FROM inbound_email WHERE is_personal = 0 AND mailbox_type not like 'bounce' AND status = 'Active' AND deleted = 0 {$extraWhere}";
		$r = $this->db->query($q, true);
...
eoq;
        ...
        $meta['email']['from_addr'] = to_html($this->email->from_addr_name);
        $meta['email']['toaddrs'] = to_html($this->email->to_addrs_names);
